...
| 痛点 | 做法 |
|---|---|
| **一**：同一个场景有七八张，分不出哪张更好 | 本机感知指纹把画面相似的照片**收拢成一组**，组内在 512px 灰度图上算清晰度、反差、曝光并排序，只让最稳的一张进入候选。清晰度是**组内相对判断**——同场景里明显更糊的那张会被标出来，而本身细节就少的干净海景不会被误标。你不用逐张比，只需要确认组代表 |
| **二**：人物照和风景照没法用同一把尺子挑 | 用 Apple Vision 在本机分成**人物 / 风景两条独立赛道**，各自设定要留几张、各自排序、各自导出。不做跨类比较 |
| **三**：想让 AI 帮忙，但不想为几千张照片付钱 | 先用本机漏斗把几千张收窄到几十张，**AI 只看这几十张**：相似组里只送组代表，你已经淘汰的一张都不送。每次最多 5 张、去掉 EXIF/GPS 的低清 JPEG（512/1024/1536px 由你选），发送前确认。自带 Key，只存本机 Keychain |
