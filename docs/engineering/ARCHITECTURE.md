...
选择文件夹
   ...
   ↓  AIFinalSelectionRunPlanner             候选切成每次 1 张的传输窗口
...
8. **候选池缓存与照片状态同步。** 由 `photos` 的 `didSet` 统一作废，新增写入路径不必单独接线。
   这条曾被漏过三次（相似家族计算、演示评分逐批写入、提交新一轮评分），
   表现为"待评分长期为 0"或"评分完了还显示待评分"——所以不变量放在源头，不放在调用点。
